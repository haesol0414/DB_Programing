...
1. 모델 만들기 : 가장 위 왼쪽 초록색 아이콘 -> 새로만들기 -> 디폴트:논리 및 물리모델링 -> 모델명 입력 -> DMAS 선택

2. 엔티티 만들기 : 툴 박스에서 엔티티 클릭 -> 화면에서 더블 클릭 해서 편집창 열기 -> 엔티티 명과 속성 설정(아래 추가 버튼으로 엔티티 추가 가능, 실질 식별자와 not noll에 모두 체크를 하면 Primary Key가 됨.)

3. 서브타입 설정 : 엔티티 편집창에서 서브타입으로 설정할 엔티티 선택 후 서브타입 버튼 클릭 -> 서브타입명 설정 -> 화면에서 서브타입 더블 클릭 -> 속성명과 속성 설정


** 각 엔티티 속성 아이콘별 의미
	# : 실질 식별자 (PK)
	(#) : 보조 식별자 (보조 식별자로 설정하면 인덱스를 통해 접근 가능한 애트리뷰트가 됨)
	* : not null 속성이 있음
	o : 기본
...
** 엔티티 타입의 선정 방법
- 업무 기술서, 장표, 인터뷰 정리문서 등에서 명사를 구분.
- 개념이 불분명한 것과 광범위한 것은 제거한다.
- 엔티티 타입의 특성이거나 속성인 값은 제거한다.
