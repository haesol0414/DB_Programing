...
*** 3 Week ***
	...
	** 3단계 데이터 베이스 설계
	- 1. 현실세계 문제와 엔티티의 관계로 개념적 설계 -> 2. 논리 ER-D 이용 논리적 설계 -> 3. 논리스키마(릴레이션)로 물리적 설계
	- 물리적 설계를 마치면 테이블들의 집합이 데이터베이스에 저장됨.
	- 논리적 설계 = DA#5 사용, 물리적 설계 = ERwin으로 DDL Script 생성 (자동)
	- DDL Script를 Oracle SQL Developer을 이용하여 불러온 후 실행해볼 수 있음.


	* ERwin 사용하기
	- 새 창 만들기 : file - new - logical/physical - oracle - version 9x
	- 표기법 변경 : model - model properties - nodation - IE - IE
 	- 엔티티 만들고 엔티티명 - tab - 기본키 - tab - 애트리뷰트 생성
 	- 서브타입 만들기 : 엔티티 만들기 - Exclusive sub-category (부모 엔티티의 PK가 서브타입 엔티티의 FK로 내려옴)
 	- 저장 : file - save


 	* 바커 표기법과 IE 표기법
 	- 바커 표기법에서 실선은 1을 의미하고, 점선은 0,1에 해당한다.
 	- IE 표기법에서 실선은 식별관계(부모 pk가 자식 엔티티에 PK로 상속되는 것)를 의미하고, 점선은 비식별관계(부모 pk가 자식 엔티티에 일반 속성으로 상속되는 것)을 의미한다.
 	- IE 표기법에서 o 모양은 optional으로, 참여도가 '0' 즉 필수가 아닌 선택임을 의미한다. ( = Null이 가능하다.)
 	- | 는 참여도가 '1'임을 의미한다. 바커 표기법에서는 상속을 의미한다.
 	- <, > 모양은 바커 표기법과 동일하게 '다'를 의미한다.
...
- 1. 메뉴에서 속성 선택
- 2. 서브타입의 속성 클릭 후 복사
- 3. 수퍼타입의 속성에서 단축메뉴 오른쪽 클릭 후 Exclusive로 변경
- 4. 서브타입의 속성을 수퍼타입의 속성으로 드래그 앤 드롭


* 자기 참조 관계 (DAsP Guide p.305)
- 자기 실체를 참조하는 순환 관계 구조


* Arcr 관계 (상호 배타 관계)
- 어떤 엔티티가 두 개 이상의 다른 엔티티의 합칩합과 관계를 가지는 것을 배타적 관계 혹은 아크(Arc)관계 라고 한다.
- 배타적 관계는 항상 mendatory이거나 optional 이어야 한다.


** 정규화 (Normalization)
* 함수적 종속성
- 함수적 종속성 : 데이터들이 어떤 기준 값에의해 항상 종속되는 현상
- A가 B에 함수적으로 종속될 경우 A->B로 표기
- 함수적 종속성의 종류 : 
완전 함수적 종속성 (모든 PK에 의해 결정됨)
부분 함수적 종속성 (여러개의 PK 중 하나의 PK에 의해 결정됨)
이행 함수적 종속성 (두 개의 속성을 거쳐서 종속되는 경우)


* 이상 현상(Anomaly)
- 데이터의 중복 현상으로 릴레이션을 조직할 때 발생하는 비합리적 현상
- 정규화가 되지 않아 하나의 엔티티에 여러 실체의 속성들을 혼합한 경우 발생
- 이상 현상의 종류 :
삽입 이상 (특정 투플을 삽입하려고 할 때 불필요햔 데이터도 함께 삽입해야 하는 현상)
삭제 이상 (특정 투플을 삭제할 경우 유지되어야 할 정보까지도 삭제되는 연쇄 삭제 현상)
갱신 이상 (특정 속성값 갱신시에 중복 저장되어있는 속상값 중 하나만 갱신하고 나머지는 갱신하지 않아 발생하는 데이터 불일치 현상)


* 정규화의 정의와 장점
- 이상 현상을 야기하는 속성간의 종속 관계를 제거하기 위해 엔티티를 작은 여러 엔티티로 무손실 분해하는 과정 (최적화 과정)
- 데이터가 불필요하게 반복되는 일을 철저히 배제한다
- 질의에 대한 응답 시간을 줄여 속도를 향상 시킨다
- 장점 : 중복값과 널값이 줄어든다, 데이터 구조의 안정성을 최대화한다, 


* 정규화의 과정 ( 최종 목표인 제 3정규형을 만족하면 정규화가 잘 된 상태 )
- 비정규 -> 제 1정규형(1NF) -> 제 2정규형(2NF) -> 제 3정규형 (3NF)
- 제 1정규형 : 모든 애트리뷰트는 하나의 값을 가져야한다, 기본키에 대해 부분 함수 종속성이 존재함
- 제 2정규형 : 부분 함수 종속성을 제거된 상태
- 제 3정규형 : 이행적 함수 종속성을 제거한 상태, 일반 속성들이 기본 키에 대해서 이행적 함수 종속성을 가지지 않음
